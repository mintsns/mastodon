...
import GplusIntersectionObserverArticleContainer from '../containers/gplus_intersection_observer_article_container';
...
export default class GplusScrollableList extends PureComponent {
  ...
  render () {
    ...
    if (isLoading || childrenCount > 0 || !emptyMessage) {
      scrollableArea = (
        <div className={classNames('gplus', 'scrollable', { fullscreen })} ref={this.setRef}>
          <div role='feed' className='item-list'>
            {prepend}

            {React.Children.map(this.props.children, (child, index) => (
              <GplusIntersectionObserverArticleContainer
                key={child.key}
                id={child.key}
                index={index}
